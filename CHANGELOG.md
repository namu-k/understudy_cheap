...
- **Dead code cleanup**: Removed ~66 unused imports and 2 dead local functions (`readTeachClarificationState`, `writeTeachClarificationState`) from 6 files across `packages/gateway/` and `packages/core/`. Reduced oxlint warnings from 66 to 14 (14 remaining are pre-existing destructured variable warnings).

### Added

- **Seam module tests**: 65 new unit tests covering 3 extracted modules — `session-history` (51 tests), `teach-internal-sessions` (9 tests), `orchestrator-prompt` (5 tests). Tests validate normalization, history timeline construction, channel support checks, prompt assembly, and policy integration.
